...
public class ParserMediaMarkt extends AParserSite implements IParserSite {
	// Кодировка сайта
	private static String CHARSET_SITE = "UTF-8";

	public ParserMediaMarkt() {
		super();
	}
...
*/
	...
	private static final String XPATH_SHORT_FEATURES = "//div[@class='description_box __box']//text()";
	// Метка полных характеристик
	private static String XPATH_FULL_FEATURES = "//div[@class='features-list-all features-list-hidden']/div[@class='list']//text()";
	...
	protected TovarInfo fillInfo()
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (attr != null && attr.getNodeValue().equals("subtitle")) {
				nameGroup = valNode;
	...
	public String getSampleLink() {
		return "https://www.mediamarkt.ru/";
	}
