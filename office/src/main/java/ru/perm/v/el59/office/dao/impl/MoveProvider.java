...
public class MoveProvider extends GenericDaoHibernateImpl<Move, Long> implements
    ...
    private final static Logger LOGGER = Logger.getLogger(String.valueOf(MoveProvider.class));
    ...
    public List<StringSummaInOut> getSumByOperation(MoveCritery critery) {
        ...
        while (iter.hasNext()) {
            row = (Object[]) iter.next();
            SummaInOut summaInOut = new SummaInOut();
...
//									critery.fromDate,critery.toDate, critery.nnum.get(0),critery.nnum.get(0),critery.nnum.get(0),critery.nnum.get(0), ));
        }
        if (!critery.abcd.equals(""))
            tovarCritery.add(Restrictions.like("abcd", critery.abcd,
                    MatchMode.ANYWHERE));
